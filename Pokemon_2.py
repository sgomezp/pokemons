...
class Pokemon:
    ...
    def attack(self, target):
        ...
        while flag:
            if self.attacks:
                if self.pk_type == target.pk_type:
                    print("Son del mismo tipo. No pueden pelear")
                    break
                else:
                    ...
                    if result <= 0:
                        ...
                        print(
                            f"{target.name} ha sobrevivido al ataque de {self.name} quedando sus Life Points en: {result} \n")
                        lp_temp = target.life_points
    ...
    while flag:
        ...
        if nombre_retador in globals() and nombre_objetivo in globals():
            ...
            while seguir not in ["s", "n"]:
                seguir = input("Caractéres no válidos. Debe introducir s/n para continuar: ").lower()
        else:
